...
comm = MPI.COMM_WORLD
rank = comm.Get_rank()
from mpi_server import MPIServer # noqa
...
meshfile = join(expanduser("~"), 'jupyter', 'mfem', 'data', 'periodic-hexagon.mesh')
...
go = True
def stop(): go = False # noqa
server = MPIServer(comm=comm, ns=globals()) # noqa
server.serve()

while go:
    step(1)
    if server.wait_for_client(0):
        server.loop()
    time.sleep(1)
